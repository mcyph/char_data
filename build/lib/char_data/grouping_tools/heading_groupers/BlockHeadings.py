...
class BlockHeadings:
    ...
    def _process_range(self, LRtn, DState, ord_):
        ...
        from_block = formatted('block', from_)
        to_block = formatted('block', to)
    ...
    def _process_codepoint(self, LRtn, DState, ord_):
        ...
        """
        block = formatted('block', ord_)
        sub_block = None

        if block and not 'CJK' in block:
            sub_block = formatted('block subname', ord_)
    ...
    def iter_sub_range(self, LRtn, LRange, DState, flag):
        from_, to = LRange
        from_sub_block = formatted('block subname', from_)
        to_sub_block = formatted('block subname', to)

...
PATTERN = compile(
    ...
    r'(?P<kangxi_numeric_id>[0-9][0-9]?[0-9]?)(?P<is_simplified>\'?) '
    r'(?P<kangxi>.*?) '
    r'(?P<definition>.*)$'
)
...
class KangxiData:
    ...
    def get_L_kangxi(self):
        ...
        for line in _KANGXI_DATA.split('\n'):
            ...
            if D['is_simplified']:
                assert not kangxi_item.numeric_id in DSimplified
                DSimplified[kangxi_item.numeric_id] = kangxi_item
            else:
                assert not kangxi_item.numeric_id in DUnknown
                DUnknown[kangxi_item.numeric_id] = kangxi_item
        ...
        LOut = []
        for numeric_id, kangxi_item in DUnknown.items():
            if numeric_id in DSimplified:
                # Traditional/Simplified variants
                LOut.append(
                    as_kind(DSimplified[numeric_id], KANGXI_SIMPLIFIED)
                )
                LOut.append(
    ...
    def get_D_indexed_by_key(self, key, kind=KANGXI_BOTH):
        ...
        for kangxi_item in self.LKangxi:
            ...
            for i in iter_:
                ...
                elif kind == KANGXI_TRADITIONAL and D['kind'] in (
                    KANGXI_BOTH, KANGXI_TRADITIONAL
                ):
                    D[i].append(kangxi_item)
                elif kind == KANGXI_SIMPLIFIED and D['kind'] in (
